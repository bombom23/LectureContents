<template>
    <div>
        ...
        <v-spacer></v-spacer>
        <v-btn tile color="teal" @click="logout">
            <v-icon left>
                ads_click
            </v-icon>
            로그아웃버튼
   ...
   computed:{
    ...
    methods: {
        onSubmit (payload) {
            if(this.$store.state.session ==null){
            const { id, pw } = payload
            axios.post('http://localhost:9999/jpamember/login', { userId: id, password: pw, auth: null })
                    .then(res => {
                        if (res.data != "") {
                ...
                alert('이미 로그인 되어있습니다.:' + this.$store.state.session.userId)
            }
        },
        showSession () {
            if (this.isLogin == true) {
                axios.post('http://localhost:9999/jpamember/needSession')
                        .then(res => {
                            if (res.data == true) {
        ...
        removeSession () {
            axios.post('http://localhost:9999/jpamember/removeSession')
                    .then(res => {
