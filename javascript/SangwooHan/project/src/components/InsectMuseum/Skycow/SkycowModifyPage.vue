...
export default {
    ...
    data(){
        return{
            id: this.Skycow.id,
            title: this.Skycow.title,
            text: this.Skycow.text,
            createDate: this.Skycow.createDate,
            boardNo: this.Skycow.boardNo,
            vuecount: this.Skycow.vuecount
        }
    },
    methods: {
      onSubmit() {
          ...
          const {text}= this
         axios.post(`http://localhost:9999/skycow/Modify/${this.Skycow.boardNo}`, {text})
           .then(() => {
               alert('수정완료' ) 
